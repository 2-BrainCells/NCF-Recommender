...
def demo_existing_user(rec_system: DyslexiaRecommendationSystem, user_id: int = 400, top_k: int = 10):
    try:
        ...
        #     for i, rec in enumerate(recommendations_data['recommendations'][:5], 1):
        #         rec_type = "🎯" if rec.get('recommendation_type') == 'personalized' else "📈"
        #         print(f"  {i}. {rec_type} {rec.get('item_code', 'Unknown')} ({rec.get('category', 'Other')}) - {rec.get('predicted_rating', 'N/A')}/5.0")
