"""
Data preprocessing functions for Dyslexia Recommendation System
"""
import numpy as np
import pandas as pd
...
class DataProcessor:
    ...
    def create_item_data(self, ratings_df: pd.DataFrame) -> pd.DataFrame:
        """Create item data with category mappings"""
    ...
    def create_ratings_data(self, ratings_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Remove users with too many missing values
        ...
        # Reset index to create proper user IDs
        ...
        # Map item codes to item IDs
        code_to_id = {code: idx for idx, code in enumerate(ratings_matrix.columns[1:])}
    ...
    def create_user_data(self, demographic_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Reset index to create user IDs
    ...
    def encode_item_features(self, item_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Create binary encoding for categories
    ...
    def extract_user_category_preferences(self, user_data: pd.DataFrame, 
        ...
        """Extract user preferences for each category"""
        ...
        # For each category, calculate user's average rating
        for category in self.category_names:
            ...
            # Apply penalty for low interaction counts
    ...
    def extend_user_item_data(self, user_data: pd.DataFrame, item_data: pd.DataFrame, 
        ...
        """Extend user and item data to match ratings data"""
        
        # Get unique users and items from ratings data
        unique_users = ratings_data['user_id'].unique()
        unique_items = ratings_data['item_id'].unique()
        
        # Filter user data to only include users present in ratings
        user_data_filtered = user_data[user_data['user_id'].isin(unique_users)]
        
        # Create a mapping for items to ensure consistency
        item_data_filtered = item_data[item_data['item_id'].isin(unique_items)]
        
        # Extend to match ratings data exactly
        ...
        # Remove ID columns as they'll be handled separately
    ...
    def split_data(self, user_data: pd.DataFrame, item_data: pd.DataFrame, 
        ...
        # Split ratings data first to maintain consistency
    ...
    def preprocess_data(self, demographic_df: pd.DataFrame, 
        ...
        # Create item data with meaningful categories
